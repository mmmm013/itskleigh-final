...
import { Play, Pause, Download, Music, Star, ArrowRight } from 'lucide-react';
...
export default function Hero() {
  ...
  useEffect(() => {
    // FETCH REAL TRACKS (No Placeholders)
    async function fetchTracks() {
      const { data } = await supabase
        .from('st1')
        .select('*')
        .order('id', { ascending: false })
        .limit(6); // Grab Top 6 latest tracks
      ...
      <div className="relative z-10 container mx-auto px-6 py-20 flex flex-col md:flex-row items-center gap-12">
        ...
        <div className="w-full md:w-1/2 space-y-8">
          ...
          <div className="flex gap-4">
             <button className="px-8 py-4 bg-[#FFD54F] text-[#3E2723] font-bold rounded-full hover:scale-105 transition shadow-xl flex items-center gap-2">
                <Music size={20} /> Listen Now
             </button>
             <button className="px-8 py-4 bg-transparent border-2 border-[#D7CCC8] text-[#D7CCC8] font-bold rounded-full hover:bg-white/5 transition">
                View Catalog
             </button>
          </div>

          {/* MICHAEL CLAY BIO (No "MC" Placeholder) */}
          <div className="pt-8 border-t border-white/10">
             <div className="flex items-center gap-4">
                ...
                <div>
                   ...
                   <p className="text-[#D7CCC8] text-sm">Pianist. Vocalist. Producer. <br/>The driving force behind the Kleigh sound.</p>
                </div>
             </div>
          </div>
        </div>

        {/* RIGHT COLUMN: REAL TRACKS */}
